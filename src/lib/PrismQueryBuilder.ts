...
class PrismaQueryBuilder<TWhereInput = any, TModel = any> {
  ...
  private paginationConfig: Required<IPaginationConfig> = {
    page: 1,
    limit: 10,
    maxLimit: 100,
  };

  private sortConfig: Required<ISortConfig> = {
  ...
  constructor(
    ...
    config?: Partial<IPaginationConfig & ISortConfig>,
  ) {
    if (config) {
   ...
   */
  filter(): this {
    const RESERVED_KEYS = ["search", "page", "limit", "sort"];
  ...
  sort(defaultField?: string, defaultOrder?: "asc" | "desc"): this {
    ...
    const sortQuery = this.getQueryParam("sort");

    if (sortQuery) {
      const [sortField, sortOrder] = String(sortQuery).split(":");
  ...
  build(): IQueryBuilderResult {
    ...
    const args: IQueryBuilderResult = {
  ...
  async execute(prismaModel: {
    ...
    count: (args: any) => Promise<number>;
  }): Promise<{ data: TModel[]; metaData: IPaginationMeta }> {
  ...
  private validatePage(page: string): number {
